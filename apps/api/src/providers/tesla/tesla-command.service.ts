...
type CommandMapping = { endpoint: string; body?: (params?: CommandParams) => Record<string, unknown> }

// Maps our command names to Tesla Fleet API endpoints
const COMMAND_MAP: Record<CommandName, CommandMapping> = {
  ...
  windows_close:     { endpoint: 'window_control', body: (p: CommandParams = {}) => ({ command: 'close', lat: p['lat'], lon: p['lon'] }) },
  climate_start:     { endpoint: 'auto_conditioning_start' },
  climate_stop:      { endpoint: 'auto_conditioning_stop' },
  set_temperature:   { endpoint: 'set_temps', body: (p: CommandParams = {}) => ({ driver_temp: p['driverTemp'], passenger_temp: p['passengerTemp'] ?? p['driverTemp'] }) },
  set_seat_heater:   { endpoint: 'remote_seat_heater_request', body: (p: CommandParams = {}) => ({ heater: p['seat'], level: p['level'] }) },
  set_seat_cooler:   { endpoint: 'remote_seat_cooler_request', body: (p: CommandParams = {}) => ({ seat_position: p['seat'], seat_cooler_level: p['level'] }) },
  ...
  cabin_overheat_protection_on: {
    ...
    body: (p: CommandParams = {}) => ({ on: true, fan_only: Boolean(p['fanOnly']) }),
  },
  cabin_overheat_protection_off: {
    endpoint: 'set_cabin_overheat_protection',
    body: () => ({ on: false }),
  },
  valet_on:          { endpoint: 'set_valet_mode', body: (p: CommandParams = {}) => ({ on: true, password: p['pin'] }) },
  valet_off:         { endpoint: 'set_valet_mode', body: () => ({ on: false }) },
  speed_limit_activate:   { endpoint: 'speed_limit_activate', body: (p: CommandParams = {}) => ({ pin: p['pin'] }) },
  speed_limit_deactivate: { endpoint: 'speed_limit_deactivate', body: (p: CommandParams = {}) => ({ pin: p['pin'] }) },
  speed_limit_clear_pin:  { endpoint: 'speed_limit_clear_pin', body: (p: CommandParams = {}) => ({ pin: p['pin'] }) },
  speed_limit_set:        { endpoint: 'speed_limit_set_limit', body: (p: CommandParams = {}) => ({ limit_mph: p['limitMph'] }) },
  software_update_schedule: { endpoint: 'schedule_software_update', body: (p: CommandParams = {}) => ({ offset_sec: p['offsetSec'] ?? 0 }) },
  software_update_cancel:   { endpoint: 'cancel_software_update' },
  navigation_gps:         { endpoint: 'navigation_gps_request', body: (p: CommandParams = {}) => ({ lat: p['lat'], lon: p['lon'], order: p['order'] }) },
  charge_start:      { endpoint: 'charge_start' },
  charge_stop:       { endpoint: 'charge_stop' },
  set_charge_limit:  { endpoint: 'set_charge_limit', body: (p: CommandParams = {}) => ({ percent: p['percent'] }) },
