...
export class TeslaEcoPolicyService {
  ...
  getPollInterval(state: string, ecoModeEnabled: boolean): number {
    if (state === 'asleep' || state === 'offline') {
      return ecoModeEnabled ? 900_000 : 120_000 // 15min eco, 2min normal
    }
    if (state === 'charging') {
      return ecoModeEnabled ? 600_000 : 15_000 // 10min eco
    }
    if (state === 'driving') {
      return ecoModeEnabled ? 120_000 : 30_000 // 2min eco
    }
    // online/idle
    return ecoModeEnabled ? 900_000 : 30_000 // 15min eco
  }
}
