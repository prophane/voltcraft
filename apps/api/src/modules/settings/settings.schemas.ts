...
export const updateSettingsSchema = z.object({
  ...
  batteryChemistry: z.enum(['nca', 'lfp']).optional(),
  batteryNominalKwh: z.number().min(10).max(250).nullable().optional(),
  tirePressureTargetBar: z.number().min(1.5).max(4).optional(),
  tirePressureToleranceBar: z.number().min(0.05).max(1).optional(),
  maxDailyDrainPct: z.number().min(0.1).max(20).optional(),
  maxRecommendedSocPct: z.number().int().min(50).max(100).optional(),
